function load(){
 ...
 function dds(){
		...
		//POST calls the dopost in servlet
		xhr.open("POST","http://localhost:9090/Project001/DSSServlet");
		var payload = jbuild(); 
		xhr.send(payload);  
	}
...
xhr.send(payload);
 ...
 function dsupdate(){
	 var obj ={};
		var elements=document.getElementsById("submitds").elements;
		for(var i = 0; i < elements.length-1; i++){
			var item = elements.item(i);
			obj[item.name]=item.value;
			console.log(obj);
...
function DSbuild(){
	 		var obj ={}; 
	 		var elements=document.getElementsById("S").elements;
	 		var jsonn = JSON.stringify(obj);
	 		console.log(jsonn);
	 		return jsonn;
	 }
