	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).isBc_approval() == false) {
					writer.append("<tr>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getParameter("Accept") != null) {
				...
				formImpl.DH_Aproval(roww, true);
				formImpl.BC_Aproval(roww, true);
				...
				formImpl.DS_Aproval(roww, true);
				formImpl.DH_Aproval(roww, true);
				formImpl.BC_Aproval(roww, false);
				RequestDispatcher rs = request.getRequestDispatcher("BenefitsCoordinator.html");
				rs.forward(request, response);
			}
		} catch (SQLException e) {
