...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		try {
			if(what.UserAuthenticationValidation(username, password) == 0) {
				System.out.println(0);
			    RequestDispatcher rs = request.getRequestDispatcher("Employee.html");
			    rs.forward(request, response);
			} else if (what.UserAuthenticationValidation(username, password) == 1) {
				System.out.println(1);
				RequestDispatcher rs = request.getRequestDispatcher("DepartmentSupervisor.html");
			    rs.forward(request, response);
			}
			else if (what.UserAuthenticationValidation(username, password) == 2) {
				System.out.println(2);
				RequestDispatcher rs = request.getRequestDispatcher("DepartmentHead.html");
			    rs.forward(request, response);
			}
			else if (what.UserAuthenticationValidation(username, password) == 3) {
